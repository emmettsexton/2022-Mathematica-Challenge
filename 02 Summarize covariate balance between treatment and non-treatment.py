...
df_wt_avg = df.groupby(["Z", "file_num"]).apply(lambda x: pd.Series(np.average(x[["Y", "V1_avg", "V2_avg", "V3_avg", "V4_avg", "V5_A_avg", "V5_B_avg", "V5_C_avg"]], weights = x["n.patients"], axis = 0), ["Y", "V1_avg", "V2_avg", "V3_avg", "V4_avg", "V5_A_avg", "V5_B_avg", "V5_C_avg"])).reset_index()
...
for i, (ax, variable) in enumerate(zip(axes.flatten(), df_diffs.variable.unique())):
    ...
    df_diffs['std_diff'].plot.density(color = 'k', alpha = 0.5, ax = ax)
